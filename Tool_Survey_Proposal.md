...
- **AI-Augmented Tool:** Google Gemini API (Tích hợp LLM để tự động lọc lỗi giả - AI triage, giải thích chi tiết lỗ hổng và tự động gợi ý code fix).
...
| Tiêu chí | Semgrep (Traditional SAST) | Google Gemini API (AI-Augmented) | OWASP ZAP (Backup DAST) | Nguồn tham chiếu |
| :--- | :--- | :--- | :--- | :--- |
| **Licence cost** | Miễn phí (Mã nguồn mở, Community Edition hoàn toàn đủ cho sinh viên). | Miễn phí hoàn toàn (Sử dụng Free Tier của Google AI Studio dành cho Developer/Sinh viên). | Miễn phí hoàn toàn (Mã nguồn mở). | [Semgrep Pricing](https://semgrep.dev/pricing), [ZAP FAQ](https://www.zaproxy.org/faq/) |
...
**Đề xuất lựa chọn:** Bộ ba Semgrep (Traditional) + OWASP ZAP (Backup/Supplementary) + Google Gemini API (AI-Augmented)
