...
| -------------------------- | ------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| Gemini 3.1 Pro, NotebookLM | Tìm nguồn và tìm hiểu DAST, OWASP ZAP, OWASP Top 10.         | Yêu cầu tìm nguồn chính thống và giải thích khái niệm DAST, ZAP, OWASP Top 10 cho seminar Security Testing.      | Nội dung nền tảng về DAST, vai trò của ZAP, cách dùng OWASP Top 10 để phân loại rủi ro và nguồn tham khảo cho tài liệu.           | Đối chiếu lại output AI với nguồn chính thống của OWASP và ZAP trước khi đưa vào user guide/slide.          |
| GPT-5.5                    | Hỗ trợ viết script cấu hình output format của ZAP scan.      | Yêu cầu xuất ZAP scan ra`json` và `html`, hỗ trợ pipeline đọc kết quả và báo cáo thủ công.                       | Draft logic chọn report format, cấu hình output path và phần metadata/tag cho kết quả scan.                                       | Đọc lại script/output, kiểm tra format JSON/HTML, bổ sung phần tag để trace với nhóm lỗi bảo mật liên quan. |
| Gemini 3.1 Pro             | Hỗ trợ tìm hiểu GUI Scan với authentication trên ZAP.        | Câu hỏi về quá trình scan authenticated bằng ZAP GUI và cấu hình trình duyệt khi scan localhost.                 | Hướng dẫn kiểm tra Firefox`about:config` để allow localhost trong quá trình cấu hình proxy/scan local.                            | Kiểm tra lại bằng thao tác thực tế trong môi trường local khi tìm hiểu GUI scan với authentication.         |
| GPT-5.5                    | Hỗ trợ viết script tạo AI triage và testcase từ kết quả ZAP. | ZAP scan output và yêu cầu tạo AI triage report/testcase có đủ các section cần thiết.                            | Draft code đọc kết quả scan, tạo triage theo alert/finding, sinh testcase/checklist kiểm chứng và chuẩn hóa section trong output. | Đọc lại code, kiểm thử script, kiểm tra output có đủ section yêu cầu và xác nhận output đạt yêu cầu.        |
| GPT-5.5                    | Tóm tắt nội dung được giao vào user guide.                   | Nội dung phần DAST/ZAP, scan workflow, AI triage và cách đọc output.                                             | Bản tóm tắt đưa vào user guide cho các nội dung Trung Kiên phụ trách.                                                             | Đọc lại, chỉnh sửa nội dung và đối chiếu với workflow/script/report trong repository.                       |
| GPT-5.5, taste skill       | Khởi tạo slide từ outline thuyết trình và user guide.        | Outline thuyết trình do Trung Kiên viết, nội dung user guide và yêu cầu dùng taste skill để cải thiện giao diện. | Draft slide deck, bố cục trình bày, phân bổ nội dung seminar và cải thiện giao diện slide.                                        | Kiểm tra lại slide, xác nhận đủ nội dung, đúng thứ tự trình bày và phù hợp với user guide/outline.          |
| GPT-5.5                    | Tạo bản nháp final report.                                   | Các nội dung đã có trong repository và yêu cầu tạo file final-report.                                            | Bản nháp final report tổng hợp các phần liên quan.                                                                                | Đọc lại và sửa thứ tự section, chỉnh path đến nội dung liên quan, bổ sung link YouTube trước khi dùng.      |
...
| Codex/ChatGPT  | Hỗ trợ hoàn thiện Semgrep AI triage, Postman testcase và user guide. | Semgrep scan result JSON,`semgrep_triage_report.md`, `semgrep_test_cases.md`, lỗi cài đặt/chạy command trên Windows, yêu cầu đồng bộ tiếng Việt và tạo flow demo Semgrep. | Format lại finding Semgrep thành report có`Tags lỗi`, source evidence, `Postman/PoC tự động`, payload mẫu; cập nhật user guide cài/chạy Semgrep; tạo flow demo từ scan đến PoC; bổ sung AI audit cho các prompt hoàn thiện. | Đọc lại source/output trong`src/semgrep`, chạy `python -m unittest src.semgrep.test_semgrep_ai_triage`, kiểm tra report không ghi actual result khi chưa test, đối chiếu test case với Postman/PoC thực tế trước khi kết luận. |
